...
describe("functional helpers", () => {
  describe("renameKeys", () => {
    it("renames a key in an object", () => {
      ...
      expect(renameKeys({ foo: "hello" }, source)).to.have
        .property("hello")
    ...
    it("doesn't change the value of the renamed key", () => {
      const source = { foo: "bar" };
      expect(renameKeys({ foo: "hello" }, source)).to.have.property(
        "hello",
        source.foo
      );
    });
  ...
  describe("pickDeep", () => {
    ...
    it("picks a deep property and outputs an object with the same key and value as the property picked", () => {
      expect(pickDeep(["foo.bar"], source)).to.deep.equal({
        bar: source.foo.bar
      });
    });
  ...
  describe("pickTree", () => {
    ...
    it("returns a shallow copy with all keys that exists on input, if they exist on source", () => {
      ...
      expect(
        pickTree({ foo: true }, { foo: { foo: 1, bar: "foo" } })
      ).to.deep.equal({ foo: { foo: 1, bar: "foo" } });
      expect(
        pickTree({ bar: true }, { bar: "lel", foo: { foo: 1, bar: "foo" } })
    ...
    it("works exactly like R.pick if passed just an array", () => {
      expect(
        pickTree(["foo", "bar"], { foo: 1, bar: 2, oob: 3 })
      ).to.deep.equal({ foo: 1, bar: 2 });
    });

    it("ignores keys on input with falsey values", () => {
      expect(
        pickTree({ foo: false, bar: true }, { foo: 1, bar: 2, oob: 3 })
      ).to.deep.equal({ bar: 2 });
    });
  });
  ...
  describe("mergeBy", () => {
    it("takes two functions, applies each one to obj, and merges the resulting object", () => {
      ...
      expect(mergeBy(R.omit(["bar"]), R.prop(["bar"]), obj)).to.deep.equal({
        foo: "bar",
        oob: "foo",
        far: "bar"
      });
      expect(
        mergeBy(R.omit(["bar"]), R.always({ lel: "wat" }), obj)
  ...
  describe("spread/flattenProp", () => {
    it("merges a prop object onto the parent object", () => {
      expect(
        spread("bar", { foo: "bar", bar: { oob: "foo", far: "bar" } })
      ).to.deep.equal({ foo: "bar", oob: "foo", far: "bar" });
      expect(
        flattenProp("bar", { foo: "bar", bar: { oob: "foo", far: "bar" } })
      ).to.deep.equal({ foo: "bar", oob: "foo", far: "bar" });
    });

    it("overwrites values on the parent object", () => {
      expect(spread("bar", { foo: "bar", bar: { foo: "foo" } })).to.deep.equal({
        foo: "foo"
      });
      expect(
        flattenProp("bar", { foo: "bar", bar: { foo: "foo" } })
